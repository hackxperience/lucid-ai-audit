...
function computeScore(cs){var tw=0,ws=0;TAXONOMY.forEach(function(t){var s=cs[t.id];if(typeof s==="number"){ws+=s*t.weight;tw+=t.weight;}});return tw>0?Math.round(ws/tw):50;}
  ...
  return(
    <div style={{borderRadius:14,border:"1px solid "+(resolved?"rgba(52,199,89,0.15)":"rgba(255,255,255,0.06)"),background:resolved?"rgba(52,199,89,0.02)":"rgba(255,255,255,0.018)",overflow:"hidden",transition:"all 0.3s",opacity:resolved?0.65:1}}>
      ...
      {exp&&<div style={{padding:"0 20px 18px",borderTop:"1px solid rgba(255,255,255,0.04)",paddingTop:14,animation:"fadeIn 0.3s"}}>
        ...
        {issue.sources&&<div style={{padding:"10px 14px",borderRadius:8,background:"rgba(129,140,248,0.04)",border:"1px solid rgba(129,140,248,0.12)",marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}><Ic name="books" size={12} color={BC}/><span style={{fontFamily:BF,fontSize:9,fontWeight:700,color:tc4,letterSpacing:0.5,textTransform:"uppercase"}}>Source Citations</span></div><div style={{fontSize:11,color:BC,fontFamily:BF}}>{issue.sources.join(" · ")}</div></div>}
...
function norm(raw){return(raw||[]).map(function(iss,i){return Object.assign({},iss,{id:iss.id||(i+1),severity:["critical","warning","info"].includes(iss.severity)?iss.severity:"info",confidence:Math.min(97,Math.max(60,iss.confidence||75)),sources:iss.sources||[]});});}
...
async function readSSE(response){
  var reader=response.body.getReader();var decoder=new TextDecoder();var text="";var buf="";
  while(true){
...
{TAXONOMY.map(function(t){var s=catScores[t.id]||0;var cl=s>=70?"#34C759":s>=45?"#FF9F0A":"#FF2D55";return(
...
{Object.entries({critical:counts.critical,warning:counts.warning,info:counts.info}).map(function([k,v]){return(
...
<div style={{fontSize:18,fontWeight:900,color:SEV[k].txt,fontFamily:DF}}>{v}</div>
